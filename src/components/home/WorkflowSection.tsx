    ...
    <section className="bg-[#f7f7f5] py-20 md:py-24">
      <div className="site-container grid gap-12 lg:grid-cols-[0.72fr_1.28fr] lg:gap-24">
        <SectionHeading title="A command is only useful when you know what it guarantees.">
          ...
          <p className="mt-5"><Link href="/docs/getting-started/" className="font-medium text-neutral-900 underline decoration-lime-500 decoration-2 underline-offset-4 hover:text-neutral-500">Start with the working project path ↗</Link></p>
        </SectionHeading>
        <div className="border-t border-neutral-300">
          {trace.map(([label, command, status, detail]) => (
            <div key={label} className="grid gap-3 border-b border-neutral-300 py-5 sm:grid-cols-[6.5rem_1fr] sm:gap-6">
              <div className="flex items-start gap-2 font-mono text-xs text-neutral-500">
                <span aria-hidden="true" className={`mt-1 h-2 w-2 shrink-0 ${status === "implemented" ? "bg-lime-600" : "bg-neutral-400"}`} />
                <span>{label}</span>
              </div>
              <div>
                <code className="block break-words text-sm text-neutral-950">{command}</code>
