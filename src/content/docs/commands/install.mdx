...
The install command can add one package or install the dependencies already declared in `package.json`. It resolves the supported npm-compatible subset, verifies registry-declared integrity, writes `tapid.lock`, stores verified package trees, and activates managed `node_modules`.
